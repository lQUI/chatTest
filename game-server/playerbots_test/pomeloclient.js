...
const logger = require('./common_funcs').logger;
...
(function() {
  ...
  var pomelo = Object.create(root.EventEmitter.prototype); // object extend from object
  root.pomelo = pomelo;
  var socket = null;
  var id = 1;
  var callbacks = {};
  ...
  pomelo.init = function(params, cb){
    ...
    socket.on('connect', function(){
      logger.debug('[pomeloclient.init] websocket connected to ' + url + '!');
      if (cb) {
        cb(socket);
      }
    });
    ...
    socket.on('ping', function() {
      logger.debug('ping at ' + SharedFuncs.currentMomentObj().format('YYYY-MM-DD HH:mm:ss').toString());
    });

    socket.on('pong', function(ms) {
      logger.debug('pong at ' + SharedFuncs.currentMomentObj().format('YYYY-MM-DD HH:mm:ss').toString() + ", delay in millis is " + ms);
    });
  ...
  id++; 
  callbacks[id] = cb;
  var sg = root.encode(id,route,msg);
    socket.send(sg);
  };
  ...
  var hasProcessedMessageIdSet = new Set();
  var processMessage = function(pomelo, msg) {
    if (msg.body && msg.body.id) {
      if (hasProcessedMessageIdSet.has(msg.body.id)) return;
      hasProcessedMessageIdSet.add(msg.body.id);
    }
    ...
    if(msg.id) {
      ...
      if(typeof cb !== 'function') {
        logger.debug('[pomeloclient.processMessage] cb is not a function for request ' + msg.id);
        return;
      }
